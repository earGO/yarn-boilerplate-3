...
function shittylize(data) {
  ...
  return {
    ...rest,
    context: data.context === true || data.context === 'SYSTEM' ? 'SYSTEM' : 'COMMON',
...
function normalize(response) {
  ...
  console.log(data)
  return data.map(
    ({ elements, metaAttributes, metaAttrs, nsiMetaAttrs, deleted, hierarchyDict, transfer, ...rest }) => {
      return {
        ...rest,
        deleted: Boolean(deleted),
        hierarchy: Boolean(hierarchyDict),
        transfer: Boolean(transfer),
        elements: (elements || []).map(({ deleted, values, id, ...rest }) => ({
          ...rest,
          elementId: id,
          deleted: Boolean(deleted),
          values: values.map(({ deleted, valueAttr, value, ...rest }) => ({
            ...rest,
            deleted: Boolean(deleted),
            value: value || valueAttr,
          })),
        })),
        attributes: (metaAttributes || metaAttrs || nsiMetaAttrs || []).map(
          ({ typeAttr, arrayAttr, required, deleted, unique, nickDictLink, ...rest }) => ({
            ...rest,
            type: typeAttr,
            link: nickDictLink,
...
export default function reducer(dicts = [], { type, payload }) {
  switch (type) {
    ...
    case success(types.LOAD_DICT_ELEMENTS): {
      const isExists = dicts.find(dict => dict.nick === payload.data.dict.nick)
